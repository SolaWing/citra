# https://github.com/DemonKingSwarn/b3DS/blob/master/b3DSDecrypt.py
...
Constant = struct.unpack('>QQ', b'\x1F\xF9\xE9\xAA\xC5\xFE\x04\x08\x02\x45\x91\xDC\x5D\x52\x76\x8A')

# Retail keys
KeyX0x18 = struct.unpack('>QQ', b'\x82\xE9\xC9\xBE\xBF\xB8\xBD\xB8\x75\xEC\xC0\xA0\x7D\x47\x43\x74')
KeyX0x1B = struct.unpack('>QQ', b'\x45\xAD\x04\x95\x39\x92\xC7\xC8\x93\x72\x4A\x9A\x7B\xCE\x61\x82')
KeyX0x25 = struct.unpack('>QQ', b'\xCE\xE7\xD8\xAB\x30\xC0\x0D\xAE\x85\x0E\xF5\xE3\x82\xAC\x5A\xF3')
KeyX0x2C = struct.unpack('>QQ', b'\xB9\x8E\x95\xCE\xCA\x3E\x4D\x17\x1F\x76\xA9\x4D\xE9\x34\xC0\x53')
...
with open(filename, 'rb') as f:
    with open(filename, 'rb+') as g:
        print(filename)  # Print the filename of the file being decrypted
                ...
                if (partition_flags[7] & 0x04):  # check if the 'NoCrypto' bit (bit 3) is set
                    ...
                    if (part_off[0] * sectorsize) > 0:  # check if partition exists
                        ...
                        if magic == b"NCCH":  # check if partition is valid
                            ...
                            romfs_sbhash = "%016X%016X%016X%016X" % (struct.unpack('>QQQQ', f.read(0x20)))
                            ...
                            if (romfs_off[0] != 0):
                                ...
                                if (romfsSizeM > 0):
                                    for i in range(romfsSizeM):
                                        ...
                                        p, i * romfsBlockSize, romfsSizeTotalMb), end='')
                                if (romfsSizeB > 0):
